  ...
  <div class="min-h-screen bg-gray-100 p-6">
    <div class="max-w-7xl mx-auto bg-white shadow rounded-xl p-6 space-y-4">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <button @click="voltar" class="text-sm bg-gray-100 px-4 py-2 rounded hover:bg-gray-200 flex items-center gap-2">
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full table-auto text-sm">
          <thead class="bg-gray-100 text-left">
            <tr>
              <!-- se for um celular só aparece a referencia, o status e ações-->
...
<td class="px-4 py-2 text-right space-x-2">
               <button
                 v-if="compra.status === 'AGUARDANDO_PAGAMENTO'"
                 @click="cancelarCompra(compra)"
                 class="text-red-600 hover:underline"
               >
                 Cancelar
               </button>
  <button @click="verDetalhes(compra)" class="text-blue-600 hover:underline">
    detalhes
  </button>
</td>
...
const buscarCompras = async () => {
  ...
  try {
    const res = await axios.get('http://localhost:8084/api/sales/me', {
...
const cancelarCompra = async (compra) => {
  const confirmado = confirm(`Deseja realmente cancelar a compra ${compra.reference_id}?`)
  if (!confirmado) return

  const token = localStorage.getItem('token')
  try {
    await axios.get(`http://localhost:8084/api/sales/cancel/${compra.ID}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    ...
    alert('Erro ao cancelar a compra.')
  }
}

const voltar = () => {
  if (!user.value) return router.push('/')
  if (user.value.role === 'admin') router.push('/admin/compras')
  else router.push('/')
}
